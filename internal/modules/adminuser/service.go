...
type repository interface {
	Create(ctx context.Context, input CreateAdminUser) (AdminUser, error)
...
// Create 创建管理员用户并保存密码哈希
func (s *Service) Create(ctx context.Context, command CreateCommand) (AdminUser, error) {
	...
	if !validUsername(command.Username) ||
		...
		utf8.RuneCountInString(command.DisplayName) > 64 {
		return AdminUser{}, NewInvalidInputError()
	}

	// 在进入持久层前生成不可逆密码哈希
	passwordHash, err := s.passwords.Hash(command.Password)
	if err != nil {
		return AdminUser{}, fmt.Errorf("hash admin user password: %w", err)
	}
	createdUser, err := s.repository.Create(ctx, CreateAdminUser{
...
func (s *Service) Login(ctx context.Context, command LoginCommand) (identity.Token, error) {
	...
	if err != nil {
		return identity.Token{}, fmt.Errorf("verify admin user password: %w", err)
	}
	if !matched {
		return identity.Token{}, NewInvalidCredentialsError()
	}
	if credential.AdminUser.Status != StatusEnabled {
		return identity.Token{}, NewDisabledError()
	}

	// 凭据和状态通过后签发 Admin 登录域会话
	return s.sessions.Issue(ctx, credential.AdminUser.ID, command.Device)
}

// GetProfile 返回指定管理员用户的公开资料
func (s *Service) GetProfile(ctx context.Context, userID string) (AdminUser, error) {
